package com.wtree.scrollortable.helper.decoration;
...
public abstract class FlexibleDividerDecoration extends RecyclerView.ItemDecoration {
    ...
    protected FlexibleDividerDecoration(Builder builder) {
        ...
        isNoFirst=builder.isNoFirstLine;
        startDraw=builder.beginPosLine;
        noDrawPos=builder.noDrawPos;
    }

    public void  setListViewOriginColor(int color){
        mOriginColor=color;
    ...
    public void setNoFirstLine(boolean value){
        isNoFirst=value;
    }

    public void setDiverColor(final int color){
        this.mColorProvider=new ColorProvider() {
            @Override
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < validChildCount; i++) {
            ...
            if(isNoFirst&&childPosition==0||childPosition<startDraw){
                continue;
            }
            if(noDrawPos!=-1&&childPosition==noDrawPos){
                continue;
            }
            ...
            switch (mDividerType) {
                ...
                case COLOR:
                    ...
                    c.drawLine(bounds.left, bounds.top, bounds.right, bounds.bottom, mPaint);
                    if(bounds.left>0&&mOriginColor!=-1){
                        mPaint.setColor(mOriginColor);
                        mOriginPaint.setStrokeWidth(mSizeProvider.dividerSize(groupIndex, parent));
                        c.drawLine(0,bounds.top,bounds.left,bounds.bottom,mOriginPaint);
                    }
                    break;
            }
        }
    ...
    public static class Builder<T extends Builder> {
        ...
        private boolean isNoFirstLine=false;
        private int beginPosLine=0;
        private int noDrawPos=-1;
        ...
        public T setNoFirst(boolean values){
            isNoFirstLine=values;
            beginPosLine=isNoFirstLine?0:1;
            return (T)this;
        }
        public T setStartLine(int start){
            beginPosLine=start;
            return (T)this;
        }
